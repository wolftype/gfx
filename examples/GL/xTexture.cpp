 ...
 *
 *    Description:  make a texture and experiment with texture coordinate
 * functions
 *
 *        Version:  1.0
...
struct MyApp : GFXApp<GLFWContext> {
  ...
  void onSetup() override {
    ...
    // data::map takes a width height and depth maps a function with inputs in
    // the range of [0,1)
    ...
    auto m = mesh::uvtex<VERTEXTYPE>(
        11, 11, .2, [](float a, float b) { return Vec2f(a, b); });
    mesh = new MBO(m);
