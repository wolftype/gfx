...
struct Eval
{
  inline static double Gaussian (double x, double b = 1, double c = 1,
                                 double a = 1)
  {
    double num = (x - b);
    double den = 2 * c * c;
...
class Stat
{
 ...
 public:
  inline static bool Gaussian (double x, double b = 1, double c = 1,
                               double a = 1)
  {
  ...
  inline static bool Prob (double p) { return (Rand::Num () <= p) ? 1 : 0; }
};
}
