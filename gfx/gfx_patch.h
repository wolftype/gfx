...
struct VPatch {
  VPatch(int _a, int _b, int _c, int _d, int _e, int _f, int _g, int _h,
         double _rw, double _rh, double _rd)
      : a(_a), b(_b), c(_c), d(_d), e(_e), f(_f), g(_g), h(_h), rw(_rw),
        rh(_rh), rd(_rd) {}
  int a, b, c, d, e, f, g, h;
  double rw, rh, rd;
};

/*! Data Structure of Neighbors in a cartesian volume (left, right, bottom, top,
 * front, back) */
class Nbr {
public:
  Nbr() {}
  ...
  Nbr(int _idx, int w, int h, int d, int type)
      : idx(_idx), xl((type & LEFT) ? -1 : _idx - (h) * (d)),
        xr((type & RIGHT) ? -1 : _idx + (h) * (d)),
        yt((type & TOP) ? -1 : _idx + d), yb((type & BOTTOM) ? -1 : _idx - d),
        zf((type & FRONT) ? -1 : _idx - 1), zb((type & BACK) ? -1 : _idx + 1),
        type(type) {}
...
public:
  Vxl() {};
  Vxl(int _a, int _b, int _c, int _d, int _e, int _f, int _g, int _h,
      int _typ = 0)
