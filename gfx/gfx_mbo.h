...
template <class T> struct TMBO {
  ...
  // problem with storing attributes here is that not all shaders will have all
  // attributes so what to do when one mbo goes through multiple shaders? maybe
  // best to keep this separate, but it is really useful for rapid prototyping
  // can call mbo.render() to use this bound attrib, or mbo.render(vatt) to call
  // some other list opengles 3.0 can just use vertex array objects . . .
  ...
  void bindAttributes(const ShaderProgram &program) {
    ...
    for (auto &i : program.attributes()) {
      attrib.add(program.id(), i.first, sizeof(T),
                 GLVertexData<T>::Init().Attribute[i.first]);
    }
    // unbind();
  }
  ...
  TMBO(MeshData<T> m, GL::USAGE usage = GL::STATIC, int id = -1) : mesh(m) {
    mCount += 1;
    idx = id == -1 ? mCount : id;
    vertex = VBO(&mesh.vertices()[0].Pos[0], mesh.num(), mesh.num() * sizeof(T),
                 GL::VERTEXBUFFER, usage);
    index = VBO(&mesh.indices()[0], mesh.numIdx(),
                mesh.numIdx() * sizeof(typename MeshData<T>::INDEXTYPE),
                GL::ELEMENTBUFFER, GL::STATIC);
  }
  ...
  void render(const VertexAttributes &vatt) {
    potentiallyUpdate();
    bind(vatt);
    drawElements();
    unbind(vatt);
  ...
  void update(Vertex *val) { vertex.update(val); }

  void update() { vertex.update(); }

  void potentiallyUpdate() {
    if (mesh.isDirty()) {
      update();
      mesh.setDirty(false);
    }
  ...
  template <class S> void update(int idx, int num, const S &val) {
 ...
 *
 *-----------------------------------------------------------------------------*/
template <class T> struct MeshBuffer {
...
template <class T> MBO &make(const T &t) {
  MeshBuffer<T>::Add(t);
 ...
 *   and pointers to Vertex Array Object that binds it, as well as
 *VertexAttributes for cases (such as OpenGLES 2) where VAO's don't exist
 *
 *   note: element array is currently fixed to static (not dynamic) ... should
 *change this
 *-----------------------------------------------------------------------------*/
...
/*             template<class T> */
/*             MBO( MeshData<T> m, GL::USAGE usage = GL::STATIC, int id = -1 ) :
 * mesh(m){ */
/*                 mCount +=1; */
/*                 idx = id == -1 ? mCount : id; */
/*                 vertex = VBO( &mesh.vertices()[0].Pos[0], mesh.num(),
 * mesh.num() * sizeof(T), GL::VERTEXBUFFER, usage ); */
/*                 index = VBO( &mesh.indices()[0], mesh.numIdx(), mesh.numIdx()
 * * sizeof(typename MeshData<T>::INDEXTYPE), GL::ELEMENTBUFFER, GL::STATIC );
 */
/*                 mode = mesh.mode(); */
/*             } */

/*             /1* MBO( Mesh m, GL::USAGE usage = GL::STATIC, int id = -1 ) :
 * mesh(m){ *1/ */
/*             /1*     mCount +=1; *1/ */
/*             /1*     idx = id == -1 ? mCount : id; *1/ */
/*             /1*     vertex = VBO( &mesh.vertices()[0].Pos[0], mesh.num(),
 * mesh.num() * sizeof(Vertex), GL::VERTEXBUFFER, usage ); *1/ */
/*             /1*     index = VBO( &mesh.indices()[0], mesh.numIdx(),
 * mesh.numIdx() * sizeof(Mesh::INDEXTYPE), GL::ELEMENTBUFFER, GL::STATIC ); *1/
 */
/*             /1*     mode = mesh.mode(); *1/ */
/*             /1*     bind(); *1/ */
/*             /1* } *1/ */
...
/*             //Enable Vertex Attributes Before Calling DrawElements */
/*             void drawElements( int num = -1, int off = 0) const { // GLenum
 * mode, */
