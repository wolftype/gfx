 ...
 *                  a rendering graph that aims to make complicated multipass
 effects like motion trace easier.
 *                  provides boolean switchability between fixed function and
 programmable OpenGL
 *
 *    loosely based on work with Graham Wakefield for the AlloSphere Research
 Group during our NanoMed project
 *
 *    GFXRenderGraph has a root render node, width and height,
 immediate/programmable, and mono/stereo. GFXApp calls its rendergraph .
 onRender () method
 *
 *    GFXRenderNode the root node, calls all upstream processes, overloads <<
 operator.  GFXAPP is a rendernode. GFXStereoNode pulls into back left and back
 right buffers (active) or resets and color masks in between (anaglyph)
      GFXFrameBufferNode
 *    GFXViewNode   sets the glViewport.  q: how to instantiate multiple with
 multiple mvp matrices.
 *    GFXShaderNode binds shader and calls upstream nodes
 *    GFXSceneNode  points to a scene (matrix manager) and sets
 modelviewprojection matrix uniform updates,
 *                  by updating downstream shader
 *                  GFXApp currently inherits from this class, but need not:
 just bind with a sceneController object...
 *    GFXMeshNode   points to a meshbuffer object and a vertex attribute list,
 updates downstream shader model transform
 *                  to do this, the GFXMeshNode::shader() method finds first
 downstream shader.
 *    @todo:
 *    GFXTextureNode points to a texture which it wraps around upstream input (a
 meshnode). downstream is a SceneNode or ShaderNode.
 *    GFXEffectsNode points a texture to a framebuffer color buffer, binds an
 MBO rect (slab) and executes a fragment shader on it
...
struct GFXRenderNode {
  ...
  GFXRenderNode &downstream() { return *mDownstream; }
  ...
  GFXShaderNode &shader() {
    ...
    if (!tmp)
      printf("error no shader downstream\n");
  ...
  /// Resize width and height and cascade upstream @todo add boolean to control
  /// cascading
  /// @todo consider not cascading, and having scenegraph manage this
  /// communication
...
template <class TA, class TB> struct GFXPairNode : GFXRenderNode {
  TA a;
  TB b;
...
/// and holds settings that are readable by all rendernodes connected to the
/// root node.
struct GFXRenderGraph : public WindowEventHandler {
  ...
  enum { MONO, ACTIVE, ANAGLYPH, SEQUENTIAL, SPLIT };

  int mStereoMode = MONO; // ANAGLYPH;
  int mGLMode = 0;        // Or IMMEDIATE
  bool bLeft = true;
  ...
  void immediate(bool b) { b ? mGLMode |= IMMEDIATE : mGLMode &= !IMMEDIATE; }
  ...
  GFXRenderGraph &root(GFXRenderNode *r) {
    mRoot = r;
    return *this;
  }

  /// initialize with width, height, GL MODE, and STEREO MODE
  void init(GFXRenderNode *r, int w, int h, int glmode = 0,
            int stereomode = MONO) {
    mGLMode = glmode;
    mStereoMode = stereomode;
    root(r);
    if (mRoot->mUpstream.empty())
      printf("warning: root render node is empty\n add nodes to root with << "
             "operator\n");
    mRoot->init(w, h, this);
  }
  ...
  virtual void onResize(int w, int h) { mRoot->resize(w, h); }

  // move this
  //     Layout layout;
  //     Scene mScene;

  //   //set the frustrum viewport @TODO move this somewhere else, to a view
  //   class void setView(float z, bool isGrid, int row=0, int col=0){
  //
  ...
  //     //If we're in multi-screen mode, RE-DO pose positions based on grid
  //     layout . . . if (isGrid) {
  //
...
struct GFXShaderNode : GFXRenderNode {
  ...
  ///  (AUTOMATE THIS with GLVertexData?)
  virtual void bindAttributes() { vatt.add<Vertex>(*program); }
  // shader id | name | size of vertex data container | offset into container of
  // each parameter
  //         program->bind();
  //           vatt.add(program->id(), "position", sizeof(Vertex), 0);
  //           vatt.add(program->id(), "sourceColor", sizeof(Vertex),
  //           Vertex::oc() ); vatt.add(program->id(), "normal", sizeof(Vertex),
  //           Vertex::on()); vatt.add(program->id(), "texCoord",
...
/// T is a GFXDrawBufferNode or a GFXFrameBufferNode or a
/// GFXFrameBufferCubeMapNode
template <class T> struct GFXStereoBufferNode : GFXRenderNode {
...
struct GFXDrawBufferNode : GFXRenderNode {
  ...
  virtual void onRender() {
    ...
    auto buf = graph().stereo()
                   ? (graph().left() ? GL_BACK_LEFT : GL_BACK_RIGHT)
                   : GL_BACK;
    glDrawBuffer(buf);
...
struct GFXSlabNode : GFXShaderNode {
  ...
  virtual void onInit() {
    program =
        new ShaderProgram(graph().useES() ? ClipSpaceVertES() : ClipSpaceVert(),
...
struct GFXFrameBufferNode : GFXRenderNode {
  ...
  virtual void onEnter() {
    fbo.bind();
    fbo.clear(); // clear viewport, if rendering Anaglyph stereo, we need to
                 // only clear left eye
    // and clear depth() for right eye -- where is this clear depth to be
    // called?
  }
  ...
  virtual void capture() {
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                             GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, texture->id(),
                             0);
      fbo.checkStatus();
      glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
      for (auto &i : mUpstream) {
...
struct GFXStereoNode : GFXRenderNode {
  ...
  virtual void onRender() {
    ...
    case GFXRenderGraph::ACTIVE: {
      ...
      graph().left(false);
      for (auto &i : mUpstream)
        i->onRender(); // i->onRender( mode | STEREO | RIGHT );
      break;
    }
...
struct GFXViewNode : GFXRenderNode {
  ...
  virtual void onInit() { viewData = new SimpleView(0, 0, 1, 1); }

  virtual void onEnter() {

    // set width and height based on downstream node, viewdata will be a
    // percentage
    if (mDownstream)
      set(mDownstream->width, mDownstream->height);

    Vec4f &v = viewData->view;
    Vec4f &c = viewData->color;

    glViewport((v[0] * width), (v[1] * height), (v[2] * width),
               (v[3] * height));
  ...
  virtual void onExit() { glDisable(GL_SCISSOR_TEST); }
};
...
struct GFXSceneNode : GFXRenderNode {
  ...
  virtual void onInit() { bResizeCascade = false; }
  ...
  /// Bind model view projection and normal matrices to downstream shader (and
  /// light position...) (uniform_if does so only if such uniforms exist)
  virtual void update() {
    ShaderProgram &dp = *shader().program;

    dp.uniform_if("lightPosition", light[0], light[1],
                  light[2]); // 2.0, 2.0, 2.0);
...
template <class T> struct GFXMeshNodeT : GFXMeshNode {

  /// pointer to MBO of type T (@TODO maybe should be a std::vector<TMBO<T>*>
  /// mbo)
  TMBO<T> *mbo;
  VertexAttributes vatt;
  ...
  void onInit() {
    ...
    for (auto &i : s.program->attributes()) {
      vatt.add(s.program->id(), i.first, sizeof(T),
               GLVertexData<T>::Attribute[i.first]); // offset( i.first ) );
    }
  }
  virtual void onRender() {
    if (graph()
            .immediate()) { // shader().immediate() ){
...
// specialize your draw routines by defining a Renderable<T>::Draw(const T& t,
// GFXSceneNode * r) method
//
//      template<class T>
//      void draw(const T& t, float r = 1.0, float g=1.0, float b=1.0, float
//      a=1.0, bool bUpdate=false){
//         if ( downstream().immediate()) {
//           render::begin(r,g,b,a);
//           render::draw(t);
//           cout << "Hello" << endl;
...
//          Renderable<T>::Draw(t,this);  ///< pass "this" to
//          Renderable<T>::Draw in order to access current model view and vertex
//          attributes
//         }
//      }
//
//      //draw many (specialize this by defining a Renderable<vector<T>>::Draw
//      method
//      //where you bind mbo and vertex attributes once, then
//      mesh::drawElements() many times) template<class T> void draw(const
//      vector<T>& t, float r=1.0, float g=1.0, float b=1.0, float a=1.0, bool
//      bUpdate =false){
//          if (downstream().immediate()){
//            render::begin(r,g,b,a);
