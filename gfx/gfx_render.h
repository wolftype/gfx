...
GFXRenderGraph &GFXRenderNode::graph ()
{
  return *mRenderGraph;
}
GFXRenderGraph GFXRenderNode::graph () const
{
  return *mRenderGraph;
}
...
struct GFXSlabNode : GFXShaderNode
{
  ...
  {
    program =
      new ShaderProgram (graph ().useES () ? ClipSpaceVertES ()
                                           : ClipSpaceVert (),
                         graph ().useES () ? TFragAlphaES () : TFragAlpha (),
                         0);
    bindAttributes ();
...
struct GFXFrameBufferNode : GFXRenderNode
{
  ...
  virtual void onEnter ()
  {
    fbo.bind ();
    fbo
      .clear ();  // clear viewport, if rendering Anaglyph stereo, we need to only clear left eye
...
struct GFXViewNode : GFXRenderNode
{
  ...
  virtual void onEnter ()
  {
    ...
    glViewport ((v[0] * width), (v[1] * height), (v[2] * width),
                (v[3] * height));
    glScissor ((v[0] * width), (v[1] * height), (v[2] * width),
               (v[3] * height));
...
template <>
inline void Renderable<MBO>::Draw (const MBO &m, GFXSceneNode *_i)
{
