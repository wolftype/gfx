...
struct Layout
{
  ...
  //
  Layout (int M = 4, int N = 4, float w = 1920, float h = 1080, float wb = 100,
          float hb = 100)
      : numRow (M),
        numCol (N),
        screenWidth (w / 100),
  ...
  int numRow;
  int numCol;  ///< Number of Rows and Columns in Multidisplay
  float screenWidth;
  float screenHeight;  ///< Width of individual screens in pixels
  float wPitch;
  float hPitch;  ///< Bezel width and height in pixels

  Vec3f speakerL, speakerR;  ///< Speaker positions

  //Total Width of MxN Display IN PIXELS or INCHES?
  float totalWidth () { return numCol * screenWidth + (numCol - 1) * wPitch; }

  //Total Height of MxN Display IN PIXELS or INCHES?
  float totalHeight () { return numRow * screenHeight + (numRow - 1) * hPitch; }
  ...
  Pose poseOf (int M, int N)
  {
    ...
    return Pose (left (M, N), bottom (M, N), 0);
  }
