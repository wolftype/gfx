...
/// Set Data Offset map
template <>
inline void GLVertexData<VertexPosition>::Make ()
{
...
/// Set Data Offset map
template <>
inline void GLVertexData<VertexColor>::Make ()
{
  Attribute["color"] = 0;
...
/// Set Data Offset map
template <>
inline void GLVertexData<VertexTexture>::Make ()
{
  Attribute["position"] = 0;
...
/// Set Data Offset map
template <>
inline void GLVertexData<VertexTexture3D>::Make ()
{
  Attribute["position"] = 0;
...
struct VertexTex3D
{
  ...
  static GLvoid *ot ()
  {
    return (GLvoid *) (2 * sizeof (Vec3f) + sizeof (Vec4f));
  }
...
template <>
inline void GLVertexData<VertexTex3D>::Make ()
{
  Attribute["position"] = 0;
...
template <>
inline void GLVertexData<Vertex>::Make ()
{
  Attribute["position"] = 0;
...
class MeshData
{
 ...
 protected:
  ...
  vector<T>
    mStore;  ///< Original Stored Copy (for transforming by absolute coordinates)
...
//set color
inline MeshData<Vertex> &color (MeshData<Vertex> &m, float r, float g, float b,
                                float a)
{
  for (int i = 0; i < m.vertex ().size (); ++i)
    {
...
//GL_IMMEDIATE_MODE DEFAULT = 1
  ...
  {  //float r = 1.0, float g = 1.0, float b = 1.0, float a = 1.0) const {
     //  glColor4f(1,1,1,1);
    GL::Begin (mMode);
    for (int i = 0; i < mIndex.size (); ++i)
      {
  ...
  for (int i = 0; i < w; ++i)
    {
      for (int j = 0; j < h; ++j)
        {
          float u =
            -tw / 2.0 + (float) i / w * tw + (bFlip ? spacing / 2.0 : 0);
