...
struct VertexPosition {
  ...
  VertexPosition() {}
  VertexPosition(const Vec3f &pos, const Vec3f &norm = Vec3f(0, 0, 1))
      : Pos(pos), Norm(norm) {}
...
struct VertexColor {
  ...
  VertexColor(const Vec4f &col = Vec4f(1, 1, 1, 1),
              const Vec2f &tex = Vec2f(0, 0))
      : Col(col), Tex(tex) {}
  VertexColor(float r, float g, float b, float a = 1.0)
      : Col(r, g, b, a), Tex(0, 0) {}
...
struct VertexTex3D {
  ...
  VertexTex3D(float x, float y, float z)
      : Pos(x, y, z), Norm(0, 0, 1), Col(1, 1, 1, 1), Tex(0.0, 0.0, 0.0) {}
  ...
  static size_t offsetTextureCoordinate() {
    return 2 * sizeof(Vec3f) + sizeof(Vec4f);
  }
  ...
  void print() { cout << Pos << Norm << Col << Tex << endl; }
};
...
/*!
   *  VERTEX DATA (Interleaved)
      Position, Normal, Color, 2D Texture Coordinate
   */
struct Vertex {
  ...
  Vertex(const Vec3f &pos = Vec3f(0, 0, 0), const Vec3f &norm = Vec3f(0, 0, 1),
         const Vec4f &col = Vec4f(1, 1, 1, 1), const Vec2f &tex = Vec2f(0, 0))
  ...
  Vertex(float x, float y, float z)
      : Pos(x, y, z), Norm(0, 0, 1), Col(1, 1, 1, 1), Tex(0.0, 0.0) {}
...
protected:
  ...
  vector<T> mStore;  ///< Original Stored Copy (for transforming by absolute
                     ///< coordinates)

  //  typedef unsigned short INDEXTYPE;   ///< Type of Index Data (max number of
  //  vertices . . .)
  vector<unsigned short> mIndex; ///< Indices of Element Array Buffer

  /// Whether it has been changed and an update to GPU is necessary
  bool bDirty = false;
...
public:
  ...
  void store() { mStore = mVertex; }
  void reset() { mVertex = mStore; }
  ...
  /// Add a vertex
  MeshData &add(const T &v) {
    mVertex.push_back(v);
    return *this;
  }
  MeshData &add(const Vec3f &v) {
    mVertex.push_back(T(v));
    return *this;
  }
  MeshData &add(const Vec3f &v, const Vec3f &n) {
    mVertex.push_back(T(v, n));
    return *this;
  }
  MeshData &add(float x, float y, float z) {
    mVertex.push_back(T(Vec3f(x, y, z)));
    return *this;
  }

  /// ADD N VERTICES
  template <typename S> MeshData &add(typename vector<S>::iterator v, int n) {
    for (int i = 0; i < n; ++i) {
      mVertex.push_back(Vertex(Vec3f(v[i][0], v[i][1], v[i][2])));
    }
  }
  ...
  virtual MeshData &color(float r, float g, float b, float a = 1.0) {
    return color(Vec4f(r, g, b, a));
  }
  virtual MeshData &color(const Vec4f &c) {
    mColor = c;
    return *this;
  }

  bool isSameColor(const Vec4f &c) {
    if (mColor[0] == c[0] && mColor[1] == c[1] && mColor[2] == c[2] &&
        mColor[3] == c[3])
      return true;
    return false;
  }
...
template <class T> MeshData<T> grid2d(int w, int h, float spacing) {
  MeshData<T> m;
...
// set color
inline MeshData<Vertex> &color(MeshData<Vertex> &m, const Vec4f &c) {
...
// immediate mode!
/* void drawVertices() const {//(float r = 1.0, float g = 1.0, float b = 1.0,
 * float a = 1.0) { */
/*     //glColor4f(r,g,b,a); */
/*     GL::Begin( mMode); */
...
// GL_IMMEDIATE_MODE DEFAULT = 1
  ...
  // immediate mode!
  void drawVertices()
      const { //(float r = 1.0, float g = 1.0, float b = 1.0, float a = 1.0) {
    // glColor4f(r,g,b,a);
    GL::Begin(mMode);
    for (int i = 0; i < mVertex.size(); ++i) {
      GL::vertex(mVertex[i].Pos);
  ...
  void drawElements() const { // float r = 1.0, float g = 1.0, float b = 1.0,
                              // float a = 1.0) const {
                              //   glColor4f(1,1,1,1);
    GL::Begin(mMode);
    for (int i = 0; i < mIndex.size(); ++i) {
  ...
  template <class T> static Mesh Point(const T &p);

  template <class T> static Mesh Points(T *p, int num);

  static Mesh Points(int num);

  template <class T> static Mesh Points(std::vector<T> p);

  // 3d vecs in 3d space . . .
  template <class T, class S> static Mesh Points2(T *p, S *q, int num);

  // 2d vecs in 3d space . . .
  template <class T, class S> static Mesh Points2D(T *p, S *q, int num);
  ...
  template <class T> static Mesh Line(T a, T b);

  // template<class T>
  // static Mesh Lines( T * a, int num);

  static Mesh Cone(double rad = 1.0, double h = 1.0, int slices = 10,
                   int stacks = 4);
  static Mesh Dir();
  ...
  static Mesh Cylinder(float r = 1.0, float h = 2.0, int slices = 20,
                       int stacks = 1);
...
inline Mesh Mesh::UV(T *p, int w, int h, float tw, float th) {
  ...
  for (int i = 0; i < w - 1; ++i) {
    for (int j = 0; j < h; ++j) {
      ...
      va.Col = Vec4f((p[a][0] + tw / 2.0) / tw, (p[a][1] + th / 2.0) / th,
                     1 - (p[a][0] + tw / 2.0) / tw, 1);
      Vertex vb(p[b][0], p[b][1], p[b][2]);
      va.Tex = Vec2f((p[b][0] + tw / 2.0) / tw, (p[b][1] + th / 2.0) / th);
      va.Col = Vec4f((p[b][0] + tw / 2.0) / tw, (p[b][1] + th / 2.0) / th,
                     1 - (p[b][0] + tw / 2.0) / tw, 1);
...
template <class T> inline Mesh Mesh::Point(const T &p) {
  Mesh m;
...
template <class T> inline Mesh Mesh::Points(T *p, int num) {
  Mesh m;
  for (int i = 0; i < num; ++i) {
...
template <class T> inline Mesh Mesh::Points(std::vector<T> p) {
  Mesh m;
...
template <class T, class S> inline Mesh Mesh::Points2(T *p, S *q, int num) {
  Mesh m;
  for (int i = 0; i < num; ++i) {
    m.add(p[i][0], p[i][1], p[i][2]).add();
...
template <class T, class S> inline Mesh Mesh::Points2D(T *p, S *q, int num) {
  Mesh m;
  for (int i = 0; i < num; ++i) {
    m.add(p[i][0], p[i][1], p[i][2]).add();
...
inline Mesh Mesh::Sphere(double rad, int slices, int stacks) {
  ...
  for (int i = 0; i < stacks - 1; ++i) {
    ...
    for (int j = 0; j < slices; ++j) {
      ...
      b = (i < stacks - 2) ? a + slices
                           : m.num() - 1; // Next Higher Latitude or North Pole
...
template <class T> inline Mesh Mesh::Line(T a, T b) {
...
inline Mesh Mesh::Cone(double r, double h, int slices, int stacks) {
  ...
  // work your way up, getting skinnier as you go
  for (int i = 0; i <= stacks; ++i) {
    ...
    for (int j = 0; j < slices; ++j) {
      ...
      // add position and normal (to do normals are wonk)
...
//           m.add (Vertex (Vec3f (x, y, z), Vec3f (x, y, z), Vec4f (1, 1, 1,
//           1),
...
inline Mesh Mesh::Frame(float size) {
  ...
  m.add(Vertex(Vec3f(size, 0, 0), Vec3f(0, 0, 1), Vec4f(1, 0, 0, 1)))
      .add()
      .add(0);
  m.add(Vertex(Vec3f(0, size, 0), Vec3f(0, 0, 1), Vec4f(0, 1, 0, 1)))
      .add()
      .add(0);
  m.add(Vertex(Vec3f(0, 0, size), Vec3f(0, 0, 1), Vec4f(0, 0, 1, 1)))
      .add()
      .add(0);
  m.mode(GL::LS);
  m.store();
...
//             Vec dir  = bFlipNormals ?  Ro::dir( vp[a] ^ vp[c] ^ vp[b]
//             ).dual() : Ro::dir( vp[a] ^ vp[b] ^ vp[c] ).dual(); return
//             dir.unit();
//     }
//
...
//     }
//
//     void drawNormals(float rr = 1, float gg = 1, float bb = 1, float aa =
//     1){
//
//         glColor4f(rr,gg,bb,aa);
//         glBegin(GL_QUADS);
...
//                 int d = c - 1;
//
//                 Vec mid = Interp::surface<Vec>( vp[a], vp[b], vp[c], vp[d],
//                 .5, .5); GL::Glyph::Line( mid, mid + normal(a,b,c) );
//                 //GL::Quad(vp[a], vp[b], vp[c], vp[d]);
//
//             }
//         }
