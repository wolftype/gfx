...
struct Slab : public GFXShaderNode {
  ...
  virtual void onRender() {
    ...
    if (mDownstream) {
      printf("slab res: %d %d\n", mDownstream->width, mDownstream->height);
...
struct RenderToTexture : GFXRenderNode {
  ...
  void onRender() {
    ...
    // do not reset glViewport here --> this will be set by downstream slab or
    // effect glViewport(0,0,root().width, root().height); //the downstream slab
    // will reset this . . .
  }
};
...
struct MotionBlur : GFXRenderNode {
  ...
  void onInit() {
    ...
    // fixme: why do slabs need to know what is downstream? because of
    // root().immediate check ... for now disabled but needs a fix for immediate
    // mode to work without mucking stuff up
  ...
  void onRender() {
    ...
    // outputMix onRender() pulls process down from upstream and displays
    // results
    outputMix.texture = rb.textureB;
    outputMix.onRender();
  }
...
//
//   void onInit(){
//     program = new ShaderProgram(makeVert(MakeOmniStereoCubeMapVert()),
//     DefaultFrag(), 0);
//
//     bindAttributes();
//
...
struct CubeSlab : GFXShaderNode {
  ...
  /**
    * @brief creates shader program, binds it and iterates through its
    attributes, adding vertex attributes to the vertex attribute array
  ...
  virtual void onInit() {
    ...
    // if there is an upstream framebuffernode, bind to its texture
    // (experimental...)
  }
  ...
  virtual void onRender() {
    ...
    for (auto &i : mUpstream)
      i->onRender(); // call upstream render to framebuffer
    if (mDownstream) {
      glViewport(0, 0, mDownstream->width, mDownstream->height);
    }
...
struct StereoScene : GFXSceneNode {
  ...
  virtual void onRender() {
    ...
    auto eyeOffset = graph().stereo()
                         ? (tmpX * eyeSep * .5) * (graph().left() ? -1 : 1)
                         : Vec3f(0, 0, 0);
