...
class Mat4
{
 public:
  ...
  {
    return Vec4<T> (row (0).dot (v), row (1).dot (v), row (2).dot (v),
                    row (3).dot (v));
  }
  ...
  Mat4 operator! ()
  {
    Mat4 &m = *this;
    double determinant =
      m.at (1, 1) * m.at (2, 2) * m.at (3, 3) * m.at (4, 4)
...
class Quat
{
 public:
  ...
  template <class T>
  Quat &set (const T &a, const T &b, const T &c, const T &d)
  {
    w = a;
    x = b;
    y = c;
...
class Color
{
 ...
 public:
  ...
  {
    os << "red: " << (int) c.r << " green: " << (int) c.g
       << " blue: " << (int) c.b << " alpha: " << (int) c.a << "\n";
    return os;
  }
};
