 ...
 *
 *    Description:  Objects you want to render need not inherit from anything --
 * instead, they just need a Rendarable<T> implementation, where T is what you
 * wanna render. This helps decouple draw routines from mathematical objects.
 *
 *        Version:  1.0
...
/*-----------------------------------------------------------------------------
 *  Super Basic Programmable Pipeline
 *-----------------------------------------------------------------------------*/
template <class T> struct RenderableBase {
  /// Look up by type
  ...
  //  static void UpdateColor(const Vec4f &c) {
  //
  //    vector<MBO> &vm = Get();
  //    for (auto &m : vm) {
  //      m.mesh.color(c[0], c[1], c[2], c[3]);
  //      m.potentiallyUpdate();
  //    }
  //  }
...
template <class T> struct ModelMatrix {
  static Mat4f Make(const T &t) {
    printf(R"(no matrix model defined for this type)");
  }
};

template <class T, int Mode = 0> struct Renderable : RenderableBase<T> {

  using Base = RenderableBase<T>;

  static void Draw(T &t, GFXSceneNode *_i) {
  ...
  static void Draw(T &t, const Mat4f &model, GFXSceneNode *_i) {
    printf(
  ...
  static void Draw(const T &t, const Mat4f &model, GFXSceneNode *_i) {
    printf(
        R"(Renderable<T>::Draw (with ModelMatrix) routine not yet defined
        for this type:
              You must delcare a Renderable<CLASSNAME>::Draw function
              See gfx/gfx_render.h)");
  }
...
template <class A, class B> inline void drawAt(const A &a, const B &p) {
  glPushMatrix();
...
template <class A, class B> inline void drawBAt(const A &a, const B &p) {
  glPushMatrix();
  glTranslatef(p[0], p[1], p[2]);
...
template <typename A> inline void draw(const A &a) {
  glPushMatrix();
  Renderable<A>::DrawImmediate(a);
  glPopMatrix();
}

template <typename A> inline void drawB(const A &a) {
  glPushMatrix();
  Renderable<A, 1>::DrawImmediate(a);
  glPopMatrix();
}
...
template <typename A> inline void pipe(const A &a, GFXSceneNode *re) {
