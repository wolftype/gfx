...
/*-----------------------------------------------------------------------------
         *  Utility function to create a string for a out vector
         *-----------------------------------------------------------------------------*/
template <int N>
string vec (string name, bool bES = false)
{
  ...
  s << N;
  string res = bES ? "lowp " : "";
  return "out " + res + "vec" + s.str () + " " + name;
}

/*-----------------------------------------------------------------------------
         *  FUNCTION TO CALCULATE NORMAL
         *-----------------------------------------------------------------------------*/
inline string NTransform ()
{
  return R"(
...
/*-----------------------------------------------------------------------------
         ...
         *-----------------------------------------------------------------------------*/
inline string VDisplaceCalcSimple ()
{
  return R"(
          ...
          *-----------------------------------------------------------------------------*/
           #version 150
            ...
            void main(void){
                ...
                vec4 texColor = texture(sampleTexture, texco);

                fragcolor = litColor;      //mix(litColor, texColor, .5);
            }
        )";
}


inline string TFragMix ()
{
  return R"(
         /*-----------------------------------------------------------------------------
...
/*-----------------------------------------------------------------------------
    ...
    *-----------------------------------------------------------------------------*/
inline string CubeClipSpaceVertES ()
{
  return R"(
...
inline string CubeClipSpaceVert ()
{
  return R"(
...
/*-----------------------------------------------------------------------------
     ...
     *-----------------------------------------------------------------------------*/
inline string DefaultVertES ()
{
  return AVertex () + VaryingES () + UMatrix () + NTransform () + VLighting ()
         + VCalc () + MVert ();
}
inline string DefaultVert ()
{
  return Version() + AVertex () + Varying () + UMatrix () + NTransform () + VLighting ()
         + VCalc () + MVert ();
}
...
{
  return AVertex () + (bES ? VaryingES () : Varying ()) + UMatrix ()
         + NTransform () + VLighting () + tCalc + MVert ();
}
...
/*-----------------------------------------------------------------------------
     ...
     *-----------------------------------------------------------------------------*/
inline string MakeOmniStereoCubeMapVert ()
{
  return R"(
