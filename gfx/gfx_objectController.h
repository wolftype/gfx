...
class ObjectController : public InputEventHandler, public WindowEventHandler
{
 ...
 public:
  ...
  template <class A>
  bool pntClicked (const A &, float rad);

  /// Mouse Movement Relative to ModelView (belongs elsewhere
  Vec3f dragCat () { return Quat::spin (mIO->drag (), !mScene->cat ()); }
  /// Axis of Mouse Movement  Relative to ModelView (belongs elsewhere
  Vec3f axisCat () { return Quat::spin (mIO->axis (), !mScene->cat ()); }
  template <class A>
  bool isSelected (A *);
  template <class A>
  void select (A *);
  ...
  struct ObjectPtr : GenericObjectPtr
  {
    ...
    virtual void store () { tObject = *mAddress; }

    virtual void touch () { transform (); }
  };
...
/*-----------------------------------------------------------------------------
     ...
     *-----------------------------------------------------------------------------*/
template <class A>
bool ObjectController::isSelected (A *a)
{
  stringstream s;
  s << a;
...
template <class A>
void ObjectController::select (A *a)
{
  stringstream s;
  s << a;
...
template <class A>
void ObjectController::deselect (A *a)
{
  stringstream s;
  s << a;
...
template <class A>
void ObjectController::toggleSelect (A *a)
{
  stringstream s;
  s << a;
...
template <class A>
bool ObjectController::pntClicked (const A &x, float rad)
{
  Vec3f v =
    io ().click ();  // [0,0] is bottom left corner [1.0,1.0] is top right
