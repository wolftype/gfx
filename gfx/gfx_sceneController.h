 ...
 *
 *    Description:  A utility for navigating virtual environment with mouse and
 * keyboard input
 *
 *    Methods here control viewing the scene, and whether we are in navigate or
 * Edit mode
 *
 *    Note methods typically work in Navigate mode, whereas Edit mode methods
 * are handled by the objectController.
 *
 *        Version:  1.0
...
/// Binding between Scene Matrix Transform Data and Context's IO (keyboard and
/// mouse)
class SceneController : public InputEventHandler {
  ...
  /// Camera Rotation and Translation Velocities, and Model Rotation Velocity
...
public:
  ...
  /// Get Reference to bound GFXio
  GFXio &io() { return *mIO; }
  /// Set Reference to bound GFXio
  void io(GFXio *io) { mIO = io; }
  /// Calculate Mouse Movements based on x and dx
  void inputCalc();
  /// Get World Space Coordinates of Mouse
  void mouseCalc();
  /// Get Viewdata, e.g. width and height
  ViewData &vd() { return io().viewdata; }
  /// Get Keyboard data, e.g. key code
  Keyboard &kd() { return io().keyboard; }
  /// Get mouse data, e.g. x, y position
  Mouse &md() { return io().mouse; }
  /// Get Scene's Camera
  Camera &camera() { return mScene->camera; }
  /// Get Scene's Model Transform
...
void SceneController::mouseCalc() {
...
/// On MouseDown, save the current mouse state, turn off vel
...
void SceneController::onMouseMove(const Mouse &m) { mouseCalc(); }
...
/// Toggles Modes.
/// If Navigate mode, then Sets the velocity and leaves it there until
/// navigationstop (onKeyUp).
...
// To do -- this exit will fail if we leave nav mode (i.e. hit tab) while moving
void SceneController::onKeyUp(const Keyboard &k) {
  if (io().mode(ControlMode::Navigate)) {
    keyboardNavigateStop();
    io().kbNavigationTriggerable = true;
  }
}
...
void SceneController::mouseModelTransform_(float acc, bool trigger) {
  ...
  // lets store the differential
  float wwx = (float)io().mouse.ddx / io().viewdata.w;
  float wwy = (float)io().mouse.ddy / io().viewdata.h;

  Vec3f axis = Vec3f(0, 0, 1).cross(Vec3f(wwx, -wwy, 0));
...
void SceneController::mouseModelTransform() {
  ...
  // spin axis is cross of z and differential
  Vec3f axis = Vec3f(0, 0, 1).cross(Vec3f(wx, -wy, 0));
  Quat quat(axis.len(), axis.unit());

  mScene->model.rot() = quat * tModelRot;

  // turn off any velocity
  model().db().set(0.0, 0.0, 0.0);
  //  Vec2f dd(io().mouse.ddx, io().mouse.ddy);
  //  camera().db() = Vec3f(0, 0, 1).cross(Vec3f(wx, -wy, 0));
  //  camera().ab() = Vec2f(wx, wy).metric();
}
