...
class VertexAttrib
{
 ...
 public:
  ...
  void enable () const
  {
    glEnableVertexAttribArray (mAtt.id);
    GL::error ("enable vattrib");
  }
  void disable () const
  {
    glDisableVertexAttribArray (mAtt.id);
    GL::error ("disable vattrib");
  }

  //Data is not sent here, just pointed to
  void pointer () const
  {
    glVertexAttribPointer (mAtt.id, mSize, mAtt.datatype, bNormal, mStride,
                           mData);
    GL::error ("pointer");
  }
