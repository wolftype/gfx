...
struct Texture
{
  ...
  Texture (int w, int h) : width (w), height (h) { init (); };
  ...
  void init ()
  {
    ...
    glTexImage2D (GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, mType,
                  NULL);
  ...
  void update (void *data)
  {
    ...
    glTexSubImage2D (GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGBA, mType,
                     data);
...
struct CubeMap
{
    ...
    for (int i = 0; i < 6; ++i)
      {
        glTexImage2D (GL_TEXTURE_CUBE_MAP_POSITIVE_X + i,  //< target
                      ...
                      mWidth, mWidth,
                      0,  //< width, height, border (cube maps must be equal)
  ...
  void bind () { glBindTexture (GL_TEXTURE_CUBE_MAP, mID); }
