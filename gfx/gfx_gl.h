...
inline void translate(float const *p) { glTranslatef(p[0], p[1], p[2]); }
inline void translate(double const *p) { glTranslated(p[0], p[1], p[2]); }
...
inline void rotate(float const *p) { glRotatef(p[0], p[1], p[2], p[3]); }

inline void rotate(double const *p) { glRotated(p[0], p[1], p[2], p[3]); }
inline void scale(float s) { glScalef(s, s, s); }
inline void scale(float sx, float sy, float sz) { glScalef(sx, sy, sz); }
inline void scale(double s) { glScaled(s, s, s); }
inline void vertex(float const *p) { glVertex3f(p[0], p[1], p[2]); }
inline void vertex(double const *p) { glVertex3d(p[0], p[1], p[2]); }

inline void normal(float const *p) { glNormal3f(p[0], p[1], p[2]); }
inline void normal(double const *p) { glNormal3d(p[0], p[1], p[2]); }

inline void color(Vec4<float> v) { glColor4f(v[0], v[1], v[2], v[3]); }
...
inline void lightOff(int i = 0) { glDisable(GL::Lights[i]); }
...
inline void specular(float r = 1.0, float g = 1.0, float b = 1.0, float a = 1.0,
                     GLenum lightnum = GL_LIGHT0) {
  GLfloat spec[] = {r, g, b, a};
  glLightfv(lightnum, GL_SPECULAR, spec);
}

inline void specular(float *r, GLenum lightnum = GL_LIGHT0) {
  glLightfv(lightnum, GL_SPECULAR, r);
}

inline void diffuse(float r = 1.0, float g = 1.0, float b = 1.0, float a = 1.0,
                    GLenum lightnum = GL_LIGHT0) {
  GLfloat diffuse[] = {r, g, b, a};
  glLightfv(lightnum, GL_DIFFUSE, diffuse);
}

inline void ambient(float r = 0, float g = 0, float b = 0, float a = 1.0,
                    GLenum lightnum = GL_LIGHT0) {
  GLfloat ambient[] = {r, g, b, a};
  glLightfv(lightnum, GL_AMBIENT, ambient);
}

inline void emission(float r = 0, float g = 0, float b = 0, float a = 1.0,
                     GLenum lightnum = GL_LIGHT0) {
  GLfloat emission[] = {r, g, b, a};
  glLightfv(lightnum, GL_EMISSION, emission);
}

inline void position(float x = 1, float y = .1, float z = .1, float w = 1.0,
                     GLenum lightnum = GL_LIGHT0) {
  GLfloat position[] = {x, y, z, w};
  glLightfv(lightnum, GL_POSITION, position);
}

inline void specularMat(float r = 1.0, float g = 1.0, float b = 1.0,
                        float a = 1.0) {
  GLfloat specmat[] = {r, g, b, a};
  glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, specmat);
}
inline void emissionMat(float r = 0.0, float g = 0.0, float b = 0.0,
                        float a = 1.0) {
