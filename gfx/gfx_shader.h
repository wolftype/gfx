...
struct ShaderParam
{
  ...
  void getUniformId (GLuint sprog) { id = glGetUniformLocation (sprog, name); }
  void getAttribId (GLuint sprog) { id = glGetAttribLocation (sprog, name); }

  void print ()
  {
    printf (" name: %s\n id: %d\n bytes: %d\n cmp: %d\n", name, id,
            GL::bpp (datatype), GL::cmp (vectype));
  }
};
...
struct Attribute : public ShaderParam
{
  ...
  Attribute (GLuint p, string n)
      : ShaderParam (ATTRIBUTE, p, glGetAttribLocation (p, n.c_str ()))
  {
  }
};
...
struct Uniform : public ShaderParam
{
  ...
  Uniform (GLuint p, string n)
      : ShaderParam (UNIFORM, p, glGetUniformLocation (p, n.c_str ()))
  {
  }
};
...
class Shader
{
 ...
 public:
  ...
  Shader (string file, Shader::Type t) : bLoaded (0), bActive (0), mType (t)
  {
    load (file, mType);
  }
  ...
  void print ()
  {
    printf ("%s:\n%s", mType == VERTEX ? "Vertex Source" : "Fragment Source",
            mSrc.c_str ());
  }
...
class ShaderProgram
{
 ...
 public:
  ...
  void
  load (const char *shaderName);  ///< Load Shader From File(.frag and .vert)
  ...
  void print ()
  {
    printSource ();
    printVariables ();
  }  ///< print both source and variables

  //unused?
  void setUniformVariable (char *name,
                           float value);  ///< Change a Variable in the Shader
  void setTexture (char *name, int id);   ///< Unused?

  /// Get ID of uniform by name
  GLint uniform (const char *name) { return mUniformMap[string (name)].id; }
  /// Get ID of attribute by name
  GLint attribute (const char *name) { return mAttributeMap[string (name)].id; }
  ...
  static const int UniformID (const char *name)
  {
    ...
    if (loc == -1)
      printf ("No such uniform named \"%s\" in shader\n", name);
...
struct ShaderData
{
  ...
  void addUniformf (const std::string &n, float v)
  {
    uniformf.push_back (Paramf (n, v));
  }
  void addUniformi (const std::string &n, int v)
  {
    uniformi.push_back (Parami (n, v));
  }
  void addAttributef (const std::string &n, float v)
  {
    attributef.push_back (Paramf (n, v));
  }
  void addAttributei (const std::string &n, int v)
  {
    attributei.push_back (Parami (n, v));
  }
  ...
  void update (ShaderProgram *shaderprogram)
  {
    ...
    for (int i = 0; i < uniformMat4f.size (); ++i)
      {
        shaderprogram->uniform (uniformMat4f[i].name.c_str (),
                                uniformMat4f[i].val);
      }
  }
};
